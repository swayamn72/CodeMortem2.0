...
import { LessonHeading, Section, P, CodeBlock, Callout, TrickTable, NavBtn, ExampleBox } from "./learn/shared/LessonHelpers";
    ...
    return (
      ...
      >
        <ChallengeIde
          ...
          nextLabelOverride={
            challenge.nextLesson === "badge" && !allLessonsComplete 
              ? "View Badge Progress →" 
              : undefined
          }
        />
    ...
    switch (activeLesson) {
      ...
      case "lesson1": return (<>
        ...
        <Section title="Positional Value — Reading Binary Numbers">
          <P>Each bit position represents a power of 2. The 0th bit is worth 2<sup>0</sup> = 1, the 1st bit is worth 2<sup>1</sup> = 2, and so on. To convert from binary to decimal, add up the values of all set bits.</P>
          <ExampleBox text={`Example: 1101 in binary
...
Add up the set bits: 8 + 4 + 0 + 1 = 13 in decimal`} />
          ...
          <ExampleBox text={`Convert 13 to binary:
...
So 13 in decimal = 1101 in binary`} />
        ...
        <Section title="Shift Operators: << and >>">
          ...
          <ExampleBox text={`Left Shift: x << k  (shift bits left by k positions)
...
Example: 5 << 3
  ...
  Notice: 5 * 2³ = 5 * 8 = 40. Left shift by k = multiply by 2ᵏ.
...
Example: 1 << 4 = 16     (binary: 10000)`} />
          ...
          <ExampleBox text={`Right Shift: x >> k  (shift bits right by k positions)
...
Example: 40 >> 3
  ...
  result:        5 in decimal
  Notice: 40 / 2³ = 40 / 8 = 5. Right shift by k = divide by 2ᵏ (floor).
...
Example: 7 >> 1
  ...
  Notice: 7 / 2 = 3.5, but integer division floors it to 3.`} />
          ...
          <Callout icon="*" color="var(--cm-cyan)">Summary: <code>x {"<<"} k</code> multiplies x by 2<sup>k</sup>. <code>x {">>"} k</code> divides x by 2<sup>k</sup> (rounding down). Shifting by k is far faster than actual multiplication or division on a CPU, but in competitive programming we use it primarily for bit manipulation patterns, not for speed.</Callout>
        </Section>
        ...
        <Section title="Three Essential Numbers">
          ...
          <ExampleBox text={`1 << k creates a "mask" with only bit k turned on:
...
or check a specific bit inside any number.`} />
          ...
          <ExampleBox text={`(1 << N) - 1 gives you a number with N bits all set to 1:
...
where all N items are selected.`} />
      ...
      case "lesson3": return (<>
        ...
        <Section title="Left Shift: Multiplying by 2ᵏ">
          <P><code>x &lt;&lt; k</code> shifts all bits k positions left, inserting zeros at the right. Equivalent to multiplying x by 2ᵏ.</P>
...
int mask = 1 << k;  // used constantly in bit manipulation`} />
        ...
        <Section title="Right Shift: Dividing by 2ᵏ">
          <P><code>x &gt;&gt; k</code> shifts all bits k positions right — floor(x / 2ᵏ) for non-negative integers.</P>
      ...
      case "lesson6": return (<>
        ...
        <Section title="XOR Identities">
          ...
          <TrickTable rows={[
            ...
            ["a ^ b = b ^ a",     "Commutative: order does not matter",                       "3⁵ = 5³ = 6"],
        ...
        <Section title="Classic Application: Find the Single Non-Duplicate">
          ...
          <ExampleBox text={`Array: [4, 1, 2, 1, 2]
...
for (int i = 0; i < n; i++)
    xp[i + 1] = xp[i] ^ a[i];`} />
          ...
          <ExampleBox text={`xp[R+1] contains: a[0] ^ a[1] ^ ... ^ a[L-1] ^ a[L] ^ ... ^ a[R]
...
That is exactly the XOR of the range [L..R].`} />
          ...
          <ExampleBox text={`Build prefix XOR array:
  xp[0] = 0
  xp[1] = 0 ^ 3         = 3
  xp[2] = 3 ^ 1         = 2
  xp[3] = 2 ^ 5         = 7
...
Using prefix array:  xp[3] ^ xp[2] = 7 ^ 2 = 5   (same answer)`} />
      ...
      case "lesson8": return (<>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", background: "rgba(255,255,255,0.02)", borderRadius: 8, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <tbody>
                {[
                  ...
                  ["Divisibility by 2ᵏ", "Shift/mask", "(n & ((1<<k) - 1)) == 0"],
        ...
        <PatternExplainer />
        <NavBtn label="Next: Common Beginner Bugs →" onClick={() => go("lesson8", "lesson10")} />
      </>);
...
long long mask = 1LL << 40; // Correct: 2⁴⁰ = 1099511627776
...
if ((x | 2) == 2) { ... }        // RIGHT`} />
        ...
        <Section title="Bug 3 — __builtin_popcount with 64-bit Values">
          <CodeBlock code={`long long x = 1000000000000000000LL; // ~10¹⁸, needs 60 bits
...
int trail = __builtin_ctzll(x);   // 64-bit trailing zeros`} />
        ...
        <Section title="Bug 4 — Shifting by the Bit Width">
...
// Bitset of 10⁵ bits, all initialized to 0
        ...
        return !allLessonsComplete ? (
          ...
          <div style={{ textAlign: "center", padding: "4rem 2rem" }}>
            ...
            <p style={{ color: "var(--text-secondary)", maxWidth: 520, margin: "0 auto 2.5rem", lineHeight: 1.85, fontSize: "0.93rem" }}>
              ...
              Kernighan&apos;s popcount, pattern recognition, common CP bugs, builtins,
